...
function FillMedications(T,medications)
   trace(medications)
   local function FillConsumable(Root,medications)
      trace(medications)
   ...
   local function FillDrug(S)
      ...
      FillConsumable(SA,medications)
      FillSymptom(SA,medications)
      FillOrder(SA,medications)
      FillDispense(SA,medications)
...
function MapMedicationsDisplay(T,medications) 

   local template = [[<table border="1" width="100%">
   <thead>
   <tr>
   <th>Medication</th>
   <th>Directions</th>
   ...
   local html = setInner(addElement(T, 'text'),template)
   ...
   for i=1, #medications do 
      ...
      cda.code.add{target=html.table.tbody,element='tr'}
      
      local tableRow = html.table.tbody:child("tr",i)
      setInner(addElement(tableRow, 'td'), medications[i].Medication)
      setInner(addElement(tableRow, 'td'), medications[i].FillInstructions)
      setInner(addElement(tableRow, 'td'), medications[i].StartDate)
      setInner(addElement(tableRow, 'td'), medications[i].Status)
      setInner(addElement(tableRow, 'td'), medications[i].Indications)
      setInner(addElement(tableRow, 'td'), medications[i].FillInstructions)
   
   end

   html.table.tbody.tr.td:insert(1,xml.ATTRIBUTE,'id')
   trace(T)
   return T
